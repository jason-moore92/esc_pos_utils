...
class Generator {
  ...
  List<int> reset() {
    ...
    _styles = PosStyles();
    bytes += setGlobalCodeTable(_codeTableString!);
  ...
  List<int> setGlobalCodeTable(String codeTableString) {
    List<int> bytes = [];
    _codeTableString = codeTableString;
    if (codeTableString != null) {
      bytes += Uint8List.fromList(
        List.from(cCodeTable.codeUnits)..add(_profile.getCodePageId(codeTableString)),
      );
      _styles = _styles.copyWith(codeTableString: codeTableString);
    }
    return bytes;
  }
  ...
  List<int> setStyles(PosStyles styles, {bool isKanji = false}) {
    ...
    if (styles.codeTableString != null) {
      bytes += Uint8List.fromList(
        List.from(cCodeTable.codeUnits)..add(_profile.getCodePageId(styles.codeTableString!)),
      );
      _styles = _styles.copyWith(align: styles.align, codeTableString: styles.codeTableString);
    } else if (_codeTableString != null) {
      bytes += Uint8List.fromList(
        List.from(cCodeTable.codeUnits)..add(_profile.getCodePageId(_codeTableString!)),
      );
      _styles = _styles.copyWith(align: styles.align, codeTableString: _codeTableString);
    }
  ...
  ///
  /// If [codeTableString] is null, global code table is used.
  /// If global code table is null, default printer code table is used.
  List<int> printCodeTable({String? codeTableString}) {
    List<int> bytes = [];
    bytes += cKanjiOff.codeUnits;
    ...
    setGlobalCodeTable(_codeTableString!);
    return bytes;
  }
