...
class Ticket {
  ...
  void row(List<PosColumn> cols) {
    ...
    for (int i = 0; i < cols.length; ++i) {
      final colInd = cols.sublist(0, i).fold(0, (int sum, col) => sum + col.width);
  ...
  /// Print a QR Code
  void qrcode(String text, {PosAlign align = PosAlign.center, QRSize size = QRSize.Size4, QRCorrection cor = QRCorrection.L}) {
    // Set alignment
    setStyles(PosStyles().copyWith(align: align));
